...
n_table_2_lucid <- sum(gt$table_figure == "table_2" & str_starts(gt$claim, "lucid"))
n_table_2_national <- n_table_2 - n_table_2_lucid
...
Every number the article prints reproduces. Of the `r n_rewrite_verifiable` claims carrying a published value, `r n_rewrite_match` match it to the precision the article reports. That includes all `r n_table_2` values of Table 2, which no deposited script computes: `r n_table_2_lucid` the rewrite recovers from the deposited `study_1.csv`, and `r n_table_2_national` from the two national surveys the article compares its sample against.
...
`r n_no_script` published quantities are not printed by any deposited script: the `r n_table_2` values of Table 2, discussed below, and `r n_no_script - n_table_2` quantities the article states in prose, mostly recruitment and quiz-passing counts that the deposit records in its data but never reports.
...
The rewrite adds `table_2_sample_comparison.R`, and all `r n_table_2` values reproduce. The Lucid column's `r n_table_2_lucid` come from the deposited `study_1.csv`, and getting them requires one undocumented recode: `educ_5` stores "Not Found" as 99 for 96 of the 1,862 respondents, and leaving those in returns a mean of 7.95 against the article's 3.01. The `r n_table_2_national` in the LNS and Pew columns are weighted means among the bilingual respondents of the two national surveys, carrying the standard error of a weighted mean rather than of an unweighted one.
...
read_csv(here::here("maintained", "output", "table_2_sample_comparison.csv"),
  ...
  left_join(
    gt |>
      ...
      separate_wider_delim(claim, ", ", names = c("column", "quantity")) |>
      select(column, quantity, paper_mean = value_paper),
    by = c("column", "quantity")
  ) |>
  mutate(across(c(mean, se), \(x) sprintf("%.2f", x))) |>
  select(column, quantity, paper_mean, mean, se, n_nonmissing) |>
  kable(
    booktabs = TRUE,
    col.names = c("Column", "Quantity", "Paper", "Rewrite mean", "Rewrite SE",
                  "Non-missing"),
    caption = "Table 2: the article's value against the rewrite, which is the only code that computes it."
  ) |>
